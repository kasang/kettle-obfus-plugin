...
public class Obfus extends BaseStep implements StepInterface {
    ...
    public Obfus(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta, Trans trans) {
    ...
    public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
         ...
         * by conan
         */
        int brithIdx = this.getInputRowMeta().indexOfValue(meta.getBrithName());
        String brithVal = getNewBrith((Date)r[brithIdx]);
        System.out.println(brithVal);
        outputRowData = RowDataUtil.addValueData(r, getInputRowMeta().size()+1, brithVal);
     ...
     * by conan
     */
    private String getNewBrith(Date date){
        char[] format = meta.getFormat().toCharArray();
        char[] vals = new SimpleDateFormat("yyyy-MM-dd").format(date).toCharArray();
        for(int i=0;i<10;i++){
            if(format[i]=='#'){
                vals[i]='#';
            }
